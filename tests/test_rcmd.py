...
class TestSceptreResolverCmd(TestCase):
    def setUp(self):
        ...
        self.envs = {"SHELL": "/my/special/shell"}
    ...
    def resolver(self):
        return SceptreResolverCmd(
            argument=self.argument, stack=self.stack, subprocess_run=self.subprocess_run
        )
    ...
    def test_resolve__dict_arg__takes_connection_params_from_dict(self):
        ...
        self.resolver.resolve()
        self.stack.connection_manager.create_session_environment_variables.assert_any_call(
            self.argument["profile"],
            self.argument["region"],
            self.argument["sceptre_role"],
        )
    ...
    def test_resolve__output_is_bytes_but_shell_encoding_not_set__deocodes_via_utf8(
        self,
    ):
        expected = "hello!"
    ...
    def test_resolve__invalid_command_argument__raises_sceptre_exception(
        self, name, arg
    ):
        self.argument = arg
